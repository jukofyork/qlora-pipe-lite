...
class PipelineModel(nn.Module):
    """
    Base pipeline model that:
    - Exposes to_layer_specs() for building DeepSpeed PipelineModule layers
    - Encapsulates weight loading and optional quantization via an internal loader
    - Annotates parameters with original_name for checkpoint compatibility
    """
    ...
    def __init__(self, config, quantization_config, model_config):
        ...
        self.module_loader = self._ModuleLoader(config['model_dir'], quantization_config, self.modules_to_not_quantize)
        for name, p in self.named_parameters():
            p.original_name = name

    def to_layer_specs(self):
        """Return a list of DeepSpeed LayerSpec/TiedLayerSpec; must be implemented by subclasses."""
        raise NotImplementedError()

    class _ModuleLoader:
        """
        Internal loader that materializes module weights (possibly sharded) and applies optional BNB quantization.
        """
        ...
        def __init__(self, model_path, quantization_config, modules_to_not_quantize):
            """Initialize loader with model path, quantization config, and non-convert module list."""
        ...
        def load_state_dict_into_module(self, module):
            """Load sharded weights into module (after optional quantization) and move to device."""
            ...
            if self.checkpoint_metadata is not None:
                ...
                weight_map = {re.sub(self.LANGUAGE_MODEL_WEIGHT_PREFIX_REGEX, '', k): v for k, v in weight_map.items()}
        ...
        def _get_partial_state_dict(self, leaf_file):
            """Load and cache a single checkpoint shard with normalized parameter names."""
            if self.loaded_state_dict is None or leaf_file != self.loaded_state_dict[0]:
                ...
                state_dict = {re.sub(self.LANGUAGE_MODEL_WEIGHT_PREFIX_REGEX, '', k): v for k, v in state_dict.items()}
                self.loaded_state_dict = (leaf_file, state_dict)
            return self.loaded_state_dict[1]

        def _maybe_quantize(self, module):
            """Optionally replace eligible Linear/Conv1d modules with BNB quantized counterparts."""
        ...
        def _recursively_replace_with_quantized_linear(
            ...
            """Recursively replace Linear/Conv1d with BNB quantized layers where allowed."""
        ...
        def _partial_module_name_match(self, full_name, list_to_match):
            """Return True if any key substring matches the module's full name."""

...
along with their ratios for each layer, plus orthogonality measure for Q.
...
if __name__ == "__main__":
    ...
    # Analyze each layer
    results = []
    print(f"{'Layer':<6} {'‖W‖_2':<8} {'‖W‖_*':<8} {'‖W‖_F':<8} {'‖W‖_2/‖W‖_*':<12} {'‖W‖_2/‖W‖_F':<12} {'‖Q^TQ-I‖_F':<12}")
    print("-" * 82)
    ...
    for layer_idx in sorted(layer_data.keys()):
        ...
        # Orthogonality measure: ||Q^T @ Q - I_r||_F
        # This measures how close Q is to being orthogonal
        QTQ = Q.t() @ Q
        I_r = torch.eye(Q.size(1), dtype=Q.dtype)
        orthogonality_error = torch.norm(QTQ - I_r, p='fro').item()
        ...
        print(f"{layer_idx:<6} "
              ...
              f"{stats['orthogonality_error']:<12.4f}")
    ...
    if results:
        ...
        print()
        print(f"Spectral norms (‖W‖_2 = max(|λ|)) --> Min: {min(spectral_norms):.3f}, Max: {max(spectral_norms):.3f}, "
              f"Mean: {sum(spectral_norms)/len(spectral_norms):.3f}")
        print(f"Nuclear norms (‖W‖_* = ||λ||_2)   --> Min: {min(nuclear_norms):.3f}, Max: {max(nuclear_norms):.3f}, "
              f"Mean: {sum(nuclear_norms)/len(nuclear_norms):.3f}")
        print(f"Frobenius norms (‖W‖_F = ||λ||_1) --> Min: {min(frobenius_norms):.3f}, Max: {max(frobenius_norms):.3f}, "
              f"Mean: {sum(frobenius_norms)/len(frobenius_norms):.3f}")
        print(f"Orthogonality errors (‖Q^TQ-I‖_F) --> Min: {min(orthogonality_errors):.4f}, Max: {max(orthogonality_errors):.4f}, "
              f"Mean: {sum(orthogonality_errors)/len(orthogonality_errors):.4f}")